...
export function registerDesignTools(server, { execute, runWrite, getProjectUuid, text, loadProjectFile, saveExportFile }) {
  ...
  server.tool('easyeda_set_schematic_pin_no_connect', 'Mark or unmark selected schematic component pins as intentionally unconnected.', {
    componentPrimitiveId:z.string().min(1), pinNumbers:z.array(z.string().min(1)).min(1),
    noConnected:z.boolean().default(true), ...commonWrite,
  }, async i=>{try{
    const code=`const componentPrimitiveId=${safeJson(i.componentPrimitiveId)},requested=${safeJson(i.pinNumbers.map(String))},noConnected=${!!i.noConnected};const pins=await eda.sch_PrimitiveComponent.getAllPinsByPrimitiveId(componentPrimitiveId);if(!Array.isArray(pins))throw new Error('EasyEDA returned no component pins');const wanted=new Set(requested);const changed=[];const available=[];for(const pin of pins){const pinNumber=String(pin?.pinNumber??pin?.getState_PinNumber?.()??'');if(pinNumber)available.push(pinNumber);if(!wanted.has(pinNumber))continue;pin.noConnected=noConnected;await pin.done();changed.push({pinNumber,noConnected:Boolean(pin.noConnected)});}const missing=requested.filter(x=>!changed.some(y=>y.pinNumber===x));if(missing.length)throw new Error('Pin number(s) not found: '+missing.join(', ')+'; available: '+available.join(', '));return {componentPrimitiveId,changed};`;
    return text(await runWrite(i,()=>execute(code,i.windowId)));
  }catch(error){return text({error:error.message},true);}});
